...
int main(int argc, char* argv[]) {
    ...
    for(int blockSizeExponent = 2; blockSizeExponent < logBaseTwo; blockSizeExponent++) {
        for(int experiment = 1; experiment < 2; experiment++) {
            ...
            */
            ...
            for ( k = 0; k < ARRAY_SIZE; k += block_size) {
                for ( j = 0; j < ARRAY_SIZE; j += block_size) {
					...
					int blocknumber = j / block_size + k  / (pow(block_size, 2) / ARRAY_SIZE);
                    std::stringstream ss;
                    ss << "\tblock " << blocknumber << " runtime " << blockruntime << " microseconds ("
                       << blockruntime / 1000000 << " seconds/" << blockruntime / 1000000 / 60 << " minutes)\n";
            ...
            std::stringstream ss;
            ss << timebuf <<  ": \t runtime " << runtime << " microseconds (" << runtime / 1000000 << " seconds/"
               << runtime / 1000000 / 60 << " minutes) for block size " << block_size << " experiment " << experiment << "\n";
            std::string logElement = ss.str();
    ...
    std::stringstream ss;
    ss << "total run time " << programruntime << " microseconds (" << programruntime / 1000000
       << " seconds/" << programruntime / 1000000 / 60 << " minutes)\n";
